...
void AGV_calc(float X_Move,float Y_Move,float Yaw ,int16_t *Speed) 
{
    ...
    cal_speed[LF_202] = sqrt(pow(Vx - Vw*arm_sin_f32(theta),2) + pow(Vy - Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1);
    cal_speed[LB_203] = sqrt(pow(Vx + Vw*arm_sin_f32(theta),2) + pow(Vy - Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1); 
    cal_speed[RB_204] = sqrt(pow(Vx + Vw*arm_sin_f32(theta),2) + pow(Vy + Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1);
