...
void AGV_calc(float X_Move,float Y_Move,float Yaw ,int16_t *Speed) 
{
    ...
    Target_velocity[RF_201] = sqrt(pow(Vx - Vw*arm_sin_f32(theta),2) + pow(Vy + Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1);
    Target_velocity[LF_202] = sqrt(pow(Vx - Vw*arm_sin_f32(theta),2) + pow(Vy - Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1);
    Target_velocity[LB_203] = sqrt(pow(Vx + Vw*arm_sin_f32(theta),2) + pow(Vy - Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1); 
    Target_velocity[RB_204] = sqrt(pow(Vx + Vw*arm_sin_f32(theta),2) + pow(Vy + Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1);
