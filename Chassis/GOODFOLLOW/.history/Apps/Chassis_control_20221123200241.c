...
void AGV_calc(float X_Move,float Y_Move,float Yaw ,int16_t *Speed) 
{
    ...
    Target_velocity[1] = sqrt(pow(Vx - Vw*arm_sin_f32(theta),2) + pow(Vy - Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1);
    Target_velocity[2] = sqrt(pow(Vx + Vw*arm_sin_f32(theta),2) + pow(Vy - Vw*arm_cos_f32(theta),2)) * (Vw>0?-1:1); 
