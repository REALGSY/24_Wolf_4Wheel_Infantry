...
void AGV_calc(float X_Move,float Y_Move,float Yaw ,int16_t *Speed) 
{
    ...
    Target_velocity[0] = sqrt(pow(X_Move - Yaw*arm_sin_f32(theta),2) + pow(Y_Move + Yaw*arm_cos_f32(theta),2)) * (Yaw>0?-1:1);
    Target_velocity[1] = sqrt(pow(X_Move - Yaw*arm_sin_f32(theta),2) + pow(Y_Move - Yaw*arm_cos_f32(theta),2)) * (Yaw>0?-1:1);
    Target_velocity[2] = sqrt(pow(X_Move + Yaw*arm_sin_f32(theta),2) + pow(Y_Move - Yaw*arm_cos_f32(theta),2)) * (Yaw>0?-1:1); 
    Target_velocity[3] = sqrt(pow(X_Move + Yaw*arm_sin_f32(theta),2) + pow(Y_Move + Yaw*arm_cos_f32(theta),2)) * (Yaw>0?-1:1);
